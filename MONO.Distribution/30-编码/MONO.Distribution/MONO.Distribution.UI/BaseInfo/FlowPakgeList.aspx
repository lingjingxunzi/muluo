<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FlowPakgeList.aspx.cs"
    Inherits="MONO.Distribution.UI.BaseInfo.FlowPakgeList" %>
    ...
    <div class="rightinfo">
        ...
        <asp:GridView ID="gvFlowPacketList" runat="server" AutoGenerateColumns="False" Width="100%"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="流量包类型">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="标准价(积分)">
                    <ItemStyle HorizontalAlign="Left"></ItemStyle>
                    <ItemTemplate>
                        <%# Convert.ToDecimal(Eval("StandardPrice") ).ToString("N2")%>
                    </ItemTemplate>
                </asp:TemplateField>
                ...
                <asp:TemplateField HeaderText="运行状态">
                    <ItemTemplate>
                        <%#Convert.ToString(Eval("ChannelStatus")) == "Y" ? "正常" : "<span style='color:Red'>维护</span>"%>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="接口并行">
                    <ItemTemplate>
                        <%#Convert.ToString(Eval("IsInterParallel")) == "Y" ? "<span style='color:#FF00FF'>并行</span>" : "串行"%>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="失败循环">
                    <ItemTemplate>
                        <%#Convert.ToString(Eval("IsRecyle")) == "Y" ? "<span style='color:#9F35FF'>循环</span>" : "单次"%>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="操作">
                    <ItemTemplate>
                        ...
                        <asp:LinkButton ID="LinkButton4" CommandName="ChannelStatus" CommandArgument='<%#Eval("FlowKey")%> '
                            runat="server" OnClientClick="return (confirm('确定改变此通道状态？'));"><%# Convert.ToString(Eval("ChannelStatus")) == "Y" ? "维护" : "正常"%>
                        </asp:LinkButton>
                        ...
                        <asp:LinkButton ID="LinkButton1" runat="server" Text="新增接口" OnClientClick='<%#Eval("FlowKey","javascript:openWindow(\"流量包接口添加\",\"FlowCarrierEdit.aspx?Command=Insert&FlowKey={0}\",\"550\",\"450\");return false;") %>'></asp:LinkButton>
                        <asp:LinkButton ID="LinkButton3" runat="server" Text="接口修改" OnClientClick='<%#Eval("FlowKey","javascript:openWindow(\"接口修改\",\"FlowCodeEdit.aspx?Command=Update&FlowKey={0}\",\"750\",\"550\");return false;") %>'></asp:LinkButton>
                        <asp:LinkButton ID="LinkButton2" runat="server" Text="接口查看" OnClientClick='<%#Eval("FlowKey","javascript:openWindow(\"接口查看\",\"FlowCodeReviewList.aspx?FlowKey={0}\",\"750\",\"550\");return false;") %>'></asp:LinkButton>
