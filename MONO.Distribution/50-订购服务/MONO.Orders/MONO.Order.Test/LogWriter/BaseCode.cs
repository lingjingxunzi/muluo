...
namespace MONO.Order.Test.LogWriter
{
    class BaseCode
    {
        ...
        private static OleDbConnection ConnectToDB()
        {
            ...
            string serverUser = configStr.Split(Convert.ToChar(";"))[2];
            string dbName = configStr.Split(Convert.ToChar(";"))[1];
            string userPwd = configStr.Split(Convert.ToChar(";"))[3];
            string strCon = "Provider=SQLNCLI10;Data Source=" + serverIP + ";User ID=" + serverUser + ";Password=" + userPwd + ";Initial Catalog=" + dbName;// + ";Trusted_Connection=yes";
        ...
        public static void WriteLog(string LogInfo)
        {
            try
            {
                LogInfo = String.Format("{0:yyyy-MM-dd hh:mm:ss}", DateTime.Now) + " :" + LogInfo;
                Console.WriteLine(LogInfo);
                string FileName = String.Format("{0:yyyyMMdd}", DateTime.Now) + "_Log.txt";
                string FileFullPath = "E://Order//" + "/log/" + FileName;
                string WriteText = string.Empty;
                if (File.Exists(FileFullPath))
                {
                    StreamWriter writer = new StreamWriter(FileFullPath, true, System.Text.Encoding.GetEncoding("GBK"));
        ...
        public static void WriteLog(string LogInfo, string flag)
        {
            try
            {
                LogInfo = String.Format("{0:yyyy-MM-dd hh:mm:ss}", DateTime.Now) + " :" + LogInfo;
                Console.WriteLine(LogInfo);
                string FileName = String.Format("{0:yyyyMMdd}", DateTime.Now) + "_" + flag + "_Log.txt";
                string FileFullPath = "E://Order//" + "/log/" + FileName;
                string WriteText = string.Empty;
                if (File.Exists(FileFullPath))
                {
